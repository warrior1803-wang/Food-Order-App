...
const cartRuducer = (state, action) => {
  switch (action.type) {
    ...
    case Actions.remove_Item:
      ...
      // if current item's amount is one, need to delete the whole item
      if (existingItemRemove.amount === 1) {
        //filter those does not match the deleted item's id
        updatedItemsRemove = state.items.filter(
          (item) => item.id !== action.id
           ...
           updatedItemsRemove[existingCartItemIndexRemove]=update
      }
